...
const CanvasPreview = forwardRef<CanvasHandle, CanvasPreviewProps>(({ side, shirtColor, width = 1024, height = 1024 }, ref) => {
  ...
  useEffect(() => {
    ...
    const loadAndDraw = async () => {
      ...
      if (side.baseImage) {
        ...
        // This is a simplified "multiply" method. 
        // For perfect results, users upload a white base.
        ctx.save();
        ctx.globalCompositeOperation = 'multiply';
        ctx.globalAlpha = 0.5;
        ctx.drawImage(shirtImg, drawX, drawY, drawW, drawH);
  ...
  const drawPlaceholderShirt = (ctx: CanvasRenderingContext2D, w: number, h: number, color: string) => {
      ...
      ctx.fillStyle = color;
      ctx.beginPath();
      // Very basic T-shape approximation for placeholder
      const padding = w * 0.1;
      ...
      // Draw a "Shirt" shape text or rect
  ...
  const drawLayer = (
    ...
    // Assuming a standard design is about 40% of canvas width at scale 1.0
