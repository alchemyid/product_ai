    ...
    try {
      ...
      if (typeof errorMessage === 'string') {
        if (errorMessage.includes('Requested entity was not found.')) {
          ...
          shouldOpenDialog = true;
        } else if (errorMessage.includes('Unsupported video generation request')) {
          userFriendlyMessage =
            'The video generation request was unsupported. Please check your settings and media inputs, as they may be incompatible with the selected mode.';
        }
      }
    ...
    if (lastConfig && lastVideoBlob && lastVideoObject) {
      try {
        ...
        const newConfig: GenerateVideoParams = {
          ...
          model: VeoModel.VEO, // Force correct model for extension
          prompt: '',
          inputVideo: videoFile,
          inputVideoObject: lastVideoObject,
          resolution: Resolution.P720,
          // Reset media types that are incompatible with extend
          startFrame: null,
          endFrame: null,
          styleImage: null,
          isLooping: false,
          referenceImages: [], // Clear reference images as they are not supported in extend mode
        };
