...
const ProductApp: React.FC = () => {
  ...
  const handleGenerateBase = async () => {
    ...
    try {
      ...
      const base64 = await geminiService.generateBaseProduct(fullPrompt, DEFAULT_ASPECT_RATIO);
  ...
  const handleGenerateVariations = useCallback(async () => {
    ...
    for (const scenario of pendingScenarios) {
      ...
      try {
        const resultBase64 = await geminiService.generateScenarioVariation(product.baseImage!, scenario.prompt, DEFAULT_ASPECT_RATIO);
