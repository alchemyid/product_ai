...
import { DEFAULT_ANGLES, DEFAULT_ASPECT_RATIO } from '@/constants/product_constants';
import geminiService from '@/services/gemini';
import { Box, Download, RefreshCw, Camera, Plus, Play, Loader2, Aperture } from 'lucide-react';
...
const ProductApp: React.FC = () => {
    ...
    const handleGenerateBase = async () => {
        ...
        try {
            ...
            const resultUri = await geminiService.generateBaseProduct(fullPrompt, DEFAULT_ASPECT_RATIO);

            // Fix: The service returns a full Data URI, but this component renders with a prefix manually.
            // We strip the prefix here to store only raw Base64, matching the upload logic.
            const rawBase64 = resultUri.includes('base64,') ? resultUri.split('base64,')[1] : resultUri;

            setProduct(prev => ({ ...prev, baseImage: rawBase64, isGeneratingBase: false }));
        } catch (error) {
            console.error(error);
    ...
    const handleGenerateVariations = useCallback(async () => {
        ...
        for (const scenario of pendingScenarios) {
            ...
            try {
                const resultUri = await geminiService.generateScenarioVariation(product.baseImage!, scenario.prompt, DEFAULT_ASPECT_RATIO);

                // Fix: Strip prefix for variations as well
                const resultBase64 = resultUri.includes('base64,') ? resultUri.split('base64,')[1] : resultUri;
